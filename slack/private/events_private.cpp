...
void initialize_events(void)
{
    ...
    slack_private::events_factory.register_type<pong>(message_channel_archive::type, [](const slack::team_id &team_id, const Json::Value &root)
    {
        return std::make_shared<message_channel_archive>(team_id, root);
    });

    slack_private::events_factory.register_type<pong>(message_channel_join::type, [](const slack::team_id &team_id, const Json::Value &root)
    {
        return std::make_shared<message_channel_join>(team_id, root);
    });
}
